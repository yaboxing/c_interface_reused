...
#define equal(s, i, t)	(memcmp(&(s).str, (t).str, (t).len) == 0)
...
char* Text_get(const* str, int size, T s)
{
...
T Text_sub(T s, int i, int j)
{
	...
	// convert i and j to indices 0 .. s.len
	assert(s.len>=0 && s.str);
	i = idx(i, s.len);
	j = idx(j, s.len);
...
int Text_rmany(T s, int i, int j, T set)
{
	...
	if(j>i && machr(set.str, s.str[j-1], set.len)) {
		do {
			--j;
...
int Text_fmt(int code, va_list *app, int put(int c, void* cl), void* cl, unsigned char flags[], int width, int precision)
{
	T* s;
...
Text_save_T Text_save()
{
	...
	alloc(1); // 一个字节的 洞 使得 isatend 无法分配任何字符串
	return save;
}
