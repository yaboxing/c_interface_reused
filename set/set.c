...
T Set_new(int hint, int com(const void * x, const void * y), unsigned hash(const void * x))
{
	T set;
	int i;
...
void Set_put(T set, const void * member)
{
	...
	} else { // FIXME 原来的 member 会造成内存泄露, 需要释放
		set->member = member;
	}
...
static T copy(T t, int hint)
{
	...
	struct member* q;
	for(i=0; i<t->size; i++){
		for(q=t->buckets[i]; q; q=q->link){ // for each member int t
			struct member* p; // add q->memner to set
			const void* member = q->member;
			int i = (*set->hash)(member) % set->size;
			NEW(p); // add member to set
			p->member = member;
			p->link = set->buckets[i];
			set->buckets[i] = p;
			set->length++;
