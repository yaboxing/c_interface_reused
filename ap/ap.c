 ...
 *|   sign|______1________|
 *|ndigits|______5________|
 *|   size|_____11________|
 *| digits|______-------------|         
 *|       |__4|245|102|_33|<--|
 *|       |___|___|___|176|	
 *			|___|___|___|
...
#define maxdigits(x, y)	((x)->ndigits>(y)->ndignts? (x)->ndigits : (y)->ndigits)
...
T AP_div(T x, T y)
{
	...
	XP_div(x->ndigits, q->digits, x->digts, y->ndigits, y->digits, r->digits, tmp);
	FREE(tmp);
	normalize(q, q->size);
	normalize(r, r->size);
	q->sign = iszero(z)||(x->sign^y->sign)==0? 1 : -1;
