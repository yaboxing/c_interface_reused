		...
		TRY
			switch(c) {
				...
				case '5': case '6': case '7': case '8': case '9': {
					...
					int i = 0;
					for(; strchr(&"zyxwvutsrqponmlkjihgfedcba9876543210"[36-ibase],h tolower(c)); c=getchar(),i++){
				...
				case 'i' : case 'o' : {
					...
					// f = obase==2||obase==8||obase==16? &u : &s;
				...
				case 'k' : {
					...
					if(n<2 || n>INT_MAX) {
						...
						Fmt_fprint(stderr, "?nonempty stask\n", s);
					} else {
						MP_set(n);
					}
				} break;
